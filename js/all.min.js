...
$(document).ready(function(){$(".note .hide").click(function(){$(this).parents('.note').hide();});$(".dontsavebutton").click(function(){item=$(this).attr('id');item=item.replace(/dontsave/,'');$("#"+item).html('<strong>editing cancelled</strong>');console.log($("#"+item).html());});$("a.ajaxable").click(function(){var url=this.href;var thispage=urlParameter('page',window.location.href);var action=urlParameter('action',url);if(action==""){url+='&action='+thispage;}
...
$('.actionresult').delay(4000).fadeOut(4000);$('.result').delay(4000).fadeOut(4000);$("#spinner").html(busyImage);$("#stopqueue").click(function(){$("#processqueueoutput").html('Processing cancelled');$("#spinner").html('&nbsp;');$("#stopqueue").hide();$("#resumequeue").show();});var docurl=document.location.search;document.cookie="browsetrail="+escape(docurl);setTimeout("autoSave();",500);$('li.selectallcategory').on('click',function(){if($(this).find('input[type=checkbox]').attr('id').match('all-lists')){var ul=$(this).parent();var lists=ul.parent().find('li');checkAllBoxes(lists.find('input[id^=all-lists]').prop('checked'),lists);}});});function allDone(text){if(!text){text='All done';}
